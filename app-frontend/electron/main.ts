...
function createWindow() {
  ...
  if (!app.isPackaged) void window.loadURL('http://127.0.0.1:5173')
  else void window.loadFile(join(frontendDir, 'dist', 'index.html'))
}

app.whenReady().then(() => {
  ipcMain.handle('unum:token', () => token)
  ipcMain.handle('unum:window', (event, action: string) => {
